...
const useCourses = () => {
    ...
    useEffect(() => {
        const fetchCourses = async () => {
            try {
                ...
                const sortedCourses = publishedCourses.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    ...
    useEffect(() => {
        if (courses.length > 0) {
            const updatedCourses = courses.map((course) => {
                ...
                if (globalDiscount) {
                    discountedPrice = course.price - (course.price * globalDiscount) / 100;
                }

                // Appliquer la remise spécifique
                const specificRemise = availableRemises.find((remise) => remise.courseId === course.id);
                if (specificRemise) {
                    discountedPrice = course.price - (course.price * specificRemise.discountPercentage) / 100;
                }
    ...
    return { courses, discountedCourses, globalDiscount, availableRemises, error };
};
