...
const Coursedetail = () => {
    ...
    useEffect(() => {
        if (course) {
            ...
            const specificRemise = availableRemises.find((remise) => remise.courseId === course.id);

            if (specificRemise) {
                finalDiscountedPrice -= (course.price * specificRemise.discountPercentage) / 100;
    ...
    useEffect(() => {
        const checkPurchase = async () => {
            try {
                if (course) {
                    const response = await axios.get(`/api/payment/check-purchase?id=${course?.id}`, { withCredentials: true });
    ...
    const countVideos = course.chapters.reduce((acc, chapter) => acc + chapter.videos.length, 0);
                ...
                <div className="order-1 col-span-1 flex flex-col space-y-6 lg:col-span-3">
                    <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }}>
                        <div className="mb-4 overflow-hidden rounded-md border p-2">
                            <div className="relative aspect-video overflow-hidden">{showImage || !course.videoUrl ? <img src={course.image || `${BASE_URL}${course.imageUrl}`} alt={course.title} className="h-full w-full object-cover" /> : <Vimeo video={course.videoUrl} responsive={true} autoplay={false} onError={handleVideoError} />}</div>
                        </div>

                        <div className="relative rounded-md border p-3">
                            <div className="mb-3 flex items-center justify-between gap-x-2">
                                <h3 className="text-xl font-semibold text-blue-gray-900 dark:text-white">{course.title}</h3>
                                <div className="focus:ring-ring inline-flex items-center rounded-md border border-transparent bg-green-500/10 px-2.5 py-0.5 text-xs font-medium text-green-800 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-book-open mr-2 h-4 w-4">
                            ...
                            <div className="my-3 text-sm text-blue-gray-500 dark:text-white">
                                <ReactQuill value={course.description} readOnly={true} theme="bubble" />
                            </div>
                            {course.chapters && course.chapters.length > 0 && (
                                <>
                                    <div className="flex items-center justify-between border-b">
                                        <Typography variant="h5" className="font-semibold text-blue-gray-900 dark:text-white">
                                            Contenu du cours
                                        </Typography>

                                        <div className="text-md flex items-center justify-end gap-x-2 border-gray-300 py-1">
                                            <p className="text-sm text-gray-800 dark:text-white">
                                                {course.chapters.length} {course.chapters.length === 1 ? "section" : "sections"}
                                            </p>
                                            <p className="text-sm text-gray-800 dark:text-white">
                                                {countVideos} {countVideos.length === 1 ? "session" : "sessions"}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="mt-4">
                                        {course.chapters.map((chapter) => (
                                            <Accordion key={chapter.id} open={open === chapter.id} icon={<Icon id={chapter.id} open={open} />}>
                                                <AccordionHeader onClick={() => handleOpen(chapter.id)} className="text-lg font-bold dark:text-white">
                                                    {chapter.title}
                                                    ...
                                                    {chapter.videos && chapter.videos.length > 0 && (
                                                        <ul className="space-y-2">
                                                            {chapter.videos.map((video) => (
                                                                <li key={video.id}>
                                                                    <button className="my-1 flex w-full items-center gap-x-2 bg-gray-50 p-1 text-sm font-bold transition hover:bg-gray-200 dark:text-black">{video.title}</button>
                                                                </li>
                                                            ))}
                                                        </ul>
                                                    )}
                    ...
                    <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }}>
                        {(user?.role && user.role === "admin") || hasPurchasedCourse ? (
                            <div className="rounded-md border bg-white p-6 shadow-md dark:bg-transparent">
                                <div className="mb-6">
                                    <h3 className="mb-4 text-xl font-semibold text-blue-gray-900 dark:text-white">Continuez là où vous vous êtes arrêté.</h3>
                                    <p className="text-sm text-blue-gray-500 dark:text-white">{progress === 0 ? "Commencez dès maintenant à visionner le cours." : "Regardez à partir du dernier chapitre terminé."}</p>
                                </div>

                                <div className="flex items-center justify-center">
                                    {progress === 0 ? (
                                        <Link to={`/course-player/course/${course.id}/chapters/${course.chapters[0].id}`} className="rounded-full">
                                            <Button variant="gradient" className="flex items-center justify-center rounded-lg px-6 py-3 transition">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-play-circle mr-2 h-4 w-4">
                                ...
                                <div className="mt-6 border-t pt-4 text-sm text-blue-gray-500 dark:text-white">
                                    <p className="flex items-center gap-x-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-info text-blue-500">
                                            <circle cx="12" cy="12" r="10"></circle>
                            ...
                            <div className="rounded-md border bg-white p-6 shadow-md dark:bg-transparent">
                                <div className="mb-6">
                                    <h3 className="mb-4 text-2xl font-semibold text-blue-gray-900 dark:text-white">Accédez à votre formation maintenant</h3>
                                    <p className="text-sm text-blue-gray-500 dark:text-white">Bénéficiez d’un accès immédiat à tous les contenus de la formation en procédant au paiement sécurisé.</p>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-center gap-x-4">
                                        {discountedPrice && discountedPrice < course.price ? (
                                            <>
                                                <span className="text-lg font-bold text-blue-gray-500 line-through dark:text-white">{course.price}€</span>
                                                <span className="text-lg font-bold text-red-600">{discountedPrice}€</span>
                                                <span className="text-sm text-green-600">({discountPercentage}% de réduction)</span>
                                            </>
                                        ) : (
                                            <span className="text-lg font-bold text-blue-gray-500 dark:text-white">{course.price}€</span>
                                        )}
                                    </div>
                                    ...
                                    {/* Bouton de paiement */}
                                    <div>
                                        <Button variant="gradient" className="checkout-button flex w-full items-center justify-center rounded-lg py-3 transition" onClick={handleCheckoutClick}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-credit-card mr-2 h-5 w-5">
                                ...
                                <div className="mt-6 border-t pt-4 text-sm text-blue-gray-600 dark:text-white">
                                    <p className="flex items-center gap-x-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-shield-check text-green-500">
