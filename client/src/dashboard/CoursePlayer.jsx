...
import { Check, File, LogOut, MoonIcon, PlayCircleIcon, SunIcon } from "lucide-react";
import CloseIcon from "@mui/icons-material/Close";
import { Accordion, AccordionHeader, AccordionBody, Button, Progress, Typography } from "@material-tailwind/react";
...
import CreateRemark from "@/dashboard/create-remark.jsx";
import CreateNote from "@/dashboard/create-note.jsx";
...
const CoursePlayer = ({ toggleTheme, theme }) => {
    ...
    useEffect(() => {
        if (!authLoading && (!isLoggedIn || !user || (user.role !== "admin" && user.role !== "user"))) {
            ...
            if (courseId) {
                ...
                try {
                    ...
                    if (res.data.lastViewedVideoId) {
                        const lastViewedVideo = res.data.chapters.flatMap((chapter) => chapter.videos).find((video) => video.id === res.data.lastViewedVideoId);
                    ...
                    const unwatchedVideo = res.data.chapters.flatMap((chapter) => chapter.videos).find((video) => !videoProgress[video.id]?.isComplete);
            ...
            try {
                for (const chapter of course.chapters) {
                    for (const video of chapter.videos) {
                        const response = await axios.get(`/api/user-progress/video/${video.id}`);
    ...
    const handleVideoEnd = async () => {
        ...
        const chapter = course.chapters.find((chapter) => chapter.videos.some((video) => video.id === selectedVideo.id));
        ...
        try {
            ...
            const courseProgressResponse = await axios.get(`/api/user-progress/${courseId}`);
    ...
    // https://vimeo.com/519509118
    // https://vimeo.com/326942270
    // https://vimeo.com/422276660
                ...
                >
                    <div className="flex h-full flex-col overflow-y-auto border-r bg-white shadow-sm dark:bg-[#020818]">
                        <div className="flex h-20 w-full flex-col space-y-2 border-b p-4">
                            ...
                            <CloseIcon className={`absolute right-4 top-3 h-6 w-6 rounded-sm text-gray-500 opacity-70 transition-colors hover:text-gray-700 md:opacity-0 ${isSidebarOpen ? "cursor-pointer" : "disabled"}`} onClick={() => setIsSidebarOpen(false)} />
                        </div>
                        <ul className="h-screen w-full overflow-auto p-2 font-medium">
                            {course.chapters.map((chapter) => (
                                <Accordion key={chapter.id} open={open === chapter.id} icon={<Icon id={chapter.id} open={open} />}>
                                    <AccordionHeader onClick={() => handleOpen(chapter.id)} className="p-3 text-lg font-bold dark:text-white">
                                        {chapter.title}
                                    </AccordionHeader>
                                ...
                                ${isCompleted ? "bg-green-400 text-white hover:bg-green-600" : selectedVideo?.id === video.id ? "bg-gray-200 text-blue-gray-900" : "text-blue-gray-800 hover:bg-gray-200"}
                              `}
                                                            >
                                                                <span className={`gap-x-1 ${isCompleted ? "text-white" : "text-blue-gray-800"}`}>{isCompleted ? <Check className="h-5 w-5" /> : <PlayCircleIcon className="h-5 w-5" />}</span>
                                                                {video.title}
                                                            </button>
                                                        </li>
                                                    );
                        ...
                        <ul className="my-5 flex justify-center">
                            <Button onClick={logout} variant="outlined" size="sm" className="flex items-center bg-white focus:outline-none">
                ...
                <div className="h-full md:pl-80">
                    <div className="sticky inset-x-0 top-0 z-40 flex h-20 w-full items-center justify-between border-b bg-white p-4 dark:bg-[#020818]">
                        <div className="flex items-center">
                            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} type="button" className="inline-flex items-center rounded-lg p-2 text-sm text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600 md:hidden">
                        ...
                        <div className="flex items-center space-x-4">
                            <Link to={"/detail/slug/" + course.slug}>
                                <Button size="sm" variant="outlined" className="flex items-center dark:bg-white dark:text-black">
                                    <ArrowBack /> Retour
                                </Button>
                            </Link>

                            <button onClick={toggleTheme} className="theme-toggle rounded-md hover:bg-gray-200">
                                {theme === "dark" ? <SunIcon className="text- h-8 w-8 p-1 text-white dark:hover:text-black" /> : <MoonIcon className="h-8 w-8 p-1" />}
                            </button>
                        </div>
                    </div>
                        ...
                        <div className="m-2">
                                        ...
                                        fallback={
                                            <div className="flex h-full w-full items-center justify-center bg-gray-100">
                                                <div className="text-center">
                                                    <div className="mb-4 text-xl font-semibold">Chargement de la vidéo...</div>
                            ...
                            <div className="mx-auto my-4">
                                <div className="flex justify-center border-b border-gray-200 py-2">
                                    {tabs.map((tab) => (
                                        <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`whitespace-nowrap px-4 py-1 text-sm font-medium focus:outline-none ${activeTab === tab.id ? "border-b-2 border-blue-gray-900 font-semibold text-blue-gray-900 dark:border-white dark:text-white" : "text-gray-500 hover:text-gray-700"}`}>
                                            {tab.label}
                                        </button>
                                    ))}
                                </div>
                                    ...
                                    {activeTab === "presentation" && (
                                        <div className="h-[600px]">
                                            <div className="grid grid-cols-1 gap-6 px-2 lg:grid-cols-2">
                                                {selectedVideo?.attachments && selectedVideo.attachments.length > 0 && (
                                                    <>
                                                        <div className="col-span-2">
                                                            <div className="mb-4">
                                                                <h3 className="text-xl font-medium">{selectedVideo.attachments.length > 1 ? "Ressources" : "Ressource"}</h3>
                                                            </div>
                                                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                                                {selectedVideo.attachments.map((attachment) => (
                                                                    <a href={`${BASE_URL}${attachment.fileUrl}`} target="_blank" rel="noopener noreferrer" key={attachment.id} className="flex items-center gap-2 rounded-md border bg-blue-50 p-2 text-blue-700 transition hover:bg-blue-100" download>
                                            ...
                                            <div className="flex items-center justify-between md:flex-row">
                                                <Typography variant="h4" className="blue-gray-900 px-2 font-bold dark:text-white">
                                                    {selectedVideo?.title}
                                                </Typography>
                                                <div className="flex gap-2">
                                                    {progress === 100 && (
                                                        <Button variant="outlined" color="red" size="sm" onClick={handleReset}>
                                                            Recommencer le cours
                                                        </Button>
                                                    )}
                                                    <Button variant="gradient" color={videoProgress[selectedVideo?.id]?.isComplete ? "blue-gray" : "green"} size="sm" onClick={handleVideoEnd} disabled={videoProgress[selectedVideo?.id]?.isComplete}>
                                                        {videoProgress[selectedVideo?.id]?.isComplete ? "Complété" : "Terminer la vidéo"}
                                                    </Button>
                                                </div>
                                            </div>

                                            <div className="mt-2 font-medium text-gray-700 dark:text-white">
                                                <ReactQuill value={course.chapters.find((chapter) => chapter.videos.some((video) => video.id === selectedVideo?.id))?.description || ""} readOnly={true} theme="bubble" />
                                            </div>
                                        </div>
                                    )}
                                    {activeTab === "qa" && <CreateRemark selectedVideo={selectedVideo} />}
                                    {activeTab === "notes" && <CreateNote selectedVideo={selectedVideo} />}
                                </div>
                            </div>
                        </div>
                    </div>
